...
enum MovementState {
	...
	IntermediateStateSentinel, # The following are intermediate states that can only be reached by teleport
...
func set_movement_state(state: MovementState, teleport := false) -> void:
	if state == MovementState.IntermediateStateSentinel || (state > MovementState.IntermediateStateSentinel && !teleport):
		push_error("Cannot transition to intermediate state %s" % MovementState.find_key(state))
		return
	
	const _movement_nodes: Array[StringName] = [
		&"Idle",
		&"Turn",
	...
	var slide_node: StringName
	var fall_node := &"End"
	var death_node := &"End"
	var deploy_node := &"End"
	match state:
		MovementState.Deploy:
			deploy_node = &"Idle"
		
		MovementState.SlideFront:
			slide_node = &"Front"
		MovementState.SlideBack:
			slide_node = &"Back"
		MovementState.SlideRight:
			slide_node = &"Right"
		MovementState.SlideLeft:
			slide_node = &"Left"
		
		MovementState.FallFront:
			fall_node = &"Front"
		...
		MovementState.DeathTorso:
			death_node = &"Torso"
		MovementState.DeathTorsoMirror:
			death_node = &"TorsoMirror"
		MovementState.DeathLegRight:
			death_node = &"LegRight"
		MovementState.DeathLegLeft:
			death_node = &"LegLeft"
		
		# Intermediate States
		MovementState.RiseFront:
			fall_node = &"RiseFront"
		MovementState.RiseBack:
			fall_node = &"RiseBack"
	...
	_play_node("parameters/playback", _movement_nodes[state], teleport)
	_play_node("parameters/Slide/playback", slide_node, teleport)
	_play_node("parameters/Fall/playback", fall_node, teleport)
	_play_node("parameters/Death/playback", death_node, teleport)
	_play_node("parameters/Deploy/playback", deploy_node, teleport)

func _play_node(playback_path: String, node_name: StringName, teleport: bool) -> void:
	if node_name.is_empty():
		return
	
	var playback := chassis_anim_tree.get(playback_path) as AnimationNodeStateMachinePlayback
	if teleport:
		playback.start(node_name)
	else:
		playback.travel(node_name)

func get_movement_playback() -> Dictionary[String,Variant]:
	...
	match movement_playback.get_current_node():
		&"Idle":
			movement_state = MovementState.Idle
		&"Turn":
			movement_state = MovementState.Turn
		&"Reverse":
			movement_state = MovementState.Reverse
		&"Walk":
			movement_state = MovementState.Walk
		&"Run":
			movement_state = MovementState.Run
		&"Wheel":
			movement_state = MovementState.Wheel
		&"Melee":
			movement_state = MovementState.Melee
		&"Deploy":
			return _get_deploy_state()
		&"Slide":
			return _get_slide_state()
		&"Fall":
			return _get_fall_state()
		&"Death":
			return _get_death_state()
		_:
			push_warning("Unknown movement_playback node: %s" % movement_playback.get_current_node())
			return {}
	
	return {
		"state": movement_state,
		"length": movement_playback.get_current_length(),
		"position": movement_playback.get_current_play_position(),
	}

func _get_deploy_state() -> Dictionary[String,Variant]:
	var deploy_playback := chassis_anim_tree.get("parameters/Deploy/playback") as AnimationNodeStateMachinePlayback
	var deploy_state: MovementState
	match deploy_playback.get_current_node():
		&"Open":
			deploy_state = MovementState.DeployOpen
		&"Idle":
			deploy_state = MovementState.Deploy
		&"Close":
			deploy_state = MovementState.DeployClose
		_:
			push_warning("Unknown deploy_playback node: %s" % deploy_playback.get_current_node())
			return {}
	
	return {
		"state": deploy_state,
		"length": deploy_playback.get_current_length(),
		"position": deploy_playback.get_current_play_position()
	}

func _get_slide_state() -> Dictionary[String,Variant]:
	var slide_playback := chassis_anim_tree.get("parameters/Slide/playback") as AnimationNodeStateMachinePlayback
	var slide_state: MovementState
	match slide_playback.get_current_node():
		&"Front":
			slide_state = MovementState.SlideFront
		&"Back":
			slide_state = MovementState.SlideBack
		&"Right":
			slide_state = MovementState.SlideRight
		&"Left":
			slide_state = MovementState.SlideLeft
		_:
			push_warning("Unknown slide_playback node: %s" % slide_playback.get_current_node())
			return {}
	
	return {
		"state": slide_state,
		"length": slide_playback.get_current_length(),
		"position": slide_playback.get_current_play_position(),
	}

func _get_fall_state() -> Dictionary[String,Variant]:
	...
	match fall_playback.get_current_node():
		&"Front":
			fall_state = MovementState.FallFront
		&"Back":
			fall_state = MovementState.FallBack
		&"Right":
			fall_state = MovementState.FallRight
		&"Left":
			fall_state = MovementState.FallLeft
		&"RiseFront":
			fall_state = MovementState.RiseFront
		&"RiseBack":
			fall_state = MovementState.RiseBack
		&"RollRight":
			fall_state = MovementState.RollRight
		&"RollLeft":
			fall_state = MovementState.RollLeft
		_:
			push_warning("Unknown fall_playback node: %s" % fall_playback.get_current_node())
			return {}
	
	return {
		"state": fall_state,
		"length": fall_playback.get_current_length(),
		"position": fall_playback.get_current_play_position(),
	}

func _get_death_state() -> Dictionary[String,Variant]:
	var death_playback := chassis_anim_tree.get("parameters/Death/playback") as AnimationNodeStateMachinePlayback
	var death_state: MovementState
	match death_playback.get_current_node():
		&"Torso":
			death_state = MovementState.DeathTorso
		&"TorsoMirror":
			death_state = MovementState.DeathTorsoMirror
		&"LegRight":
			death_state = MovementState.DeathLegRight
		&"LegLeft":
			death_state = MovementState.DeathLegLeft
		_:
			push_warning("Unknown death_playback node: %s" % death_playback.get_current_node())
			return {}
	
	return {
		"state": death_state,
		"length": death_playback.get_current_length(),
		"position": death_playback.get_current_play_position(),
	}

@export var movement_anim_speeds: Dictionary[StringName,float]
func set_movement_speed(speed_ms: float) -> void:
	for node_name in movement_anim_speeds.keys():
		chassis_anim_tree.set("parameters/%s/TimeScale/scale" % node_name, speed_ms / movement_anim_speeds[node_name])

func set_movement_turning(turning_scale: float) -> void:
	turning_scale = clampf(turning_scale, -1.0, 1.0)
	chassis_anim_tree.set("parameters/Turn/TimeScale/scale", turning_scale)
...
func get_movement_animation() -> Dictionary[String,Variant]:
	var param_path := "parameters/"
	
	var playback := chassis_anim_tree.get(param_path + "playback") as AnimationNodeStateMachinePlayback
	var state_machine := chassis_anim_tree.tree_root as AnimationNodeStateMachine
	var current_node_name := playback.get_current_node()
	
	var sub_playback := chassis_anim_tree.get(param_path + "%s/playback" % current_node_name) as AnimationNodeStateMachinePlayback
	if sub_playback:
		param_path += String(current_node_name) + "/"
		playback = sub_playback
		state_machine = state_machine.get_node(current_node_name) as AnimationNodeStateMachine
		current_node_name = sub_playback.get_current_node()
	
	var animation_node: AnimationNodeAnimation
	var animation_speed := 1.0
	
	var current_node := state_machine.get_node(current_node_name)
	if current_node is AnimationNodeAnimation:
		animation_node = current_node
	elif current_node is AnimationNodeBlendTree:
		animation_node = current_node.get_node(&"Animation") as AnimationNodeAnimation
		animation_speed = chassis_anim_tree.get(param_path + "%s/TimeScale/scale" % current_node_name) as float
	else:
		push_error("Unkown node type: ", animation_node)
		return {}
	
	return {
		"animation": chassis_anim_tree.get_animation(animation_node.animation),
		"root_motion_track": chassis_anim_tree.root_motion_track,
		"speed": animation_speed
	}

## This function should be called before get_movement_playback() or get_movement_animation() to ensure that the state machine is in a valid state
func movement_pre_advance() -> void:
	var playback := chassis_anim_tree.get("parameters/playback") as AnimationNodeStateMachinePlayback
	var sub_playback := chassis_anim_tree.get("parameters/%s/playback" % playback.get_current_node()) as AnimationNodeStateMachinePlayback
	if sub_playback:
		playback = sub_playback
	
	if playback.get_current_node() == &"Start":
		chassis_anim_tree.advance(playback.get_current_length() - playback.get_current_play_position() + 0.01)
		assert(playback.get_current_node() != &"Start")
	
	if playback.get_current_node() == &"End":
		chassis_anim_tree.advance(playback.get_current_length() - playback.get_current_play_position() + 0.01)
		assert(playback.get_current_node() != &"End")

func movement_advance(movement_position: float, animation_position: float, delta: float) -> void:
	#var playback := chassis_anim_tree.get("parameters/playback") as AnimationNodeStateMachinePlayback
	#var sub_playback := chassis_anim_tree.get("parameters/%s/playback" % playback.get_current_node()) as AnimationNodeStateMachinePlayback
	#if sub_playback:
		#playback = sub_playback
	
	var end_time := movement_position + delta
	
	# This should represent the current position within the walking animation NOT accounting for time-scale
	#var current_position := playback.get_current_play_position()
	if animation_position > end_time:
		# Current animation position is ahead of the end_time, wait for the end_time to catch up
		return
	
	# Advance the animation by the amount of time needed to sync the current_position to the end_time
	# If the current_position matches the start_time then this period is exactly equal to the delta
	chassis_anim_tree.advance(end_time - animation_position)
#endregion
